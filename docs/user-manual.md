...
![图 7 协议管理页面](images/user-manual/07-lingyun.png)

图 7 展示协议管理页面。截图中的 Broker、Provider Code、账号、设备身份和位置等运行环境信息已遮挡。

“协议管理”页面用于集中配置和调试通用 MQTT（Lingyun）与通用反制（CounterStrike）协议。列表会显示协议版本、启用和配置状态、连接状态、Broker、Client ID、设备摘要、最近活动及最近错误。页面每 5 秒自动刷新状态。

### 11.1 配置协议

1. 在协议所在行点击“配置”。
2. 在弹窗中修改连接参数、设备参数和发布间隔。
3. 如需核对协议路径，可展开设备下方的 MQTT Topic。
4. 点击“保存”提交配置，保存成功后弹窗会关闭并刷新列表。

点击“恢复默认”只会把弹窗中的字段恢复为默认值，仍需点击“保存”才会生效。配置有未保存修改时，关闭弹窗会要求确认。
...
- 是否启用协议。
- MQTT Broker、Client ID、Provider Code、Bridge Code、用户名和密码。
- SM4 Key 和 IV。
- 注册与状态上报间隔。
- 反制设备身份、名称、型号和安装位置。

开启协议前必须先保存完整配置。密码、SM4 Key 和 IV 属于敏感配置，不要在截图或问题反馈中公开。
...
协议已启用且配置完整时，可在列表中点击“测试连接”或“立即重连”。系统会清除当前重试等待、断开旧连接并立即重新连接和订阅。触发后页面会临时每秒刷新连接状态，最长持续 15 秒。

协议未启用或配置不完整时，该按钮不可用；先在配置弹窗中完成配置并保存。

### 11.4 调试工作台

1. 在协议所在行点击“调试”，工作台会在列表下方展开。
2. 选择“上报测试数据”或“下发测试命令”。
3. 选择预置报文和 Topic，也可以直接编辑 Topic 与 JSON。
4. 使用“格式化”“恢复”或“复制报文”辅助编辑。
5. CounterStrike 报文可选择明文 JSON 或 SM4-CBC Base64 加密。
6. 点击“发送”。下发模式会再次确认，因为报文会通过真实 Broker，可能触发实际设备动作。

自定义报文必须是有效 JSON。发送会使用当前协议已保存且已连接的 MQTT 会话，不提供本地模拟模式。

### 11.5 查看收发记录

工作台每 2 秒刷新一次最近 100 条内存记录。可按“全部”“发送”“接收”“失败”筛选，也可手动刷新或清空。

每条记录可复制 Topic 和逻辑 JSON。CounterStrike 加密消息还会显示并允许复制实际发送的 SM4-CBC Base64 线上报文；明文消息只显示逻辑报文。结果分为成功、失败和已忽略，失败项会显示原因。

手工下发经 Broker 返回本机订阅端后，可以依次看到手工发送、Broker 接收和控制响应。记录在修改配置和刷新页面后仍保留，但服务重启或主动清空后会消失。

### 11.6 查看 MQTT Topic

1. 打开协议的配置弹窗。
2. 在对应逻辑设备区域点击“查看 Topic”。
3. 系统会展示注册发布、状态发布、数据发布、控制订阅和控制响应 Topic。
4. 点击“收起 Topic”可关闭展示。
