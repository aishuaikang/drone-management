...
const (
	defaultMQTTTimeout = 5 * time.Second
	mqttQoS            = byte(1)
)
...
func newPahoTransport() *pahoTransport { return &pahoTransport{} }
...
func (t *pahoTransport) Connect(ctx context.Context, cfg transportConfig) error {
	...
	opts := mqtt.NewClientOptions().
		...
		SetConnectionLostHandler(func(mqtt.Client, error) { t.connected.Store(false) })
...
func (t *pahoTransport) Subscribe(ctx context.Context, topic string, handler messageHandler) error {
	...
	token := client.Subscribe(topic, mqttQoS, func(_ mqtt.Client, message mqtt.Message) {
...
func (t *pahoTransport) Publish(ctx context.Context, topic string, payload []byte) error {
	...
	token := client.Publish(topic, mqttQoS, false, payload)
	if !waitToken(ctx, token) {
...
func (t *pahoTransport) Connected() bool { return t.connected.Load() }
...
func waitToken(ctx context.Context, token mqtt.Token) bool {
	...
	case <-ctx.Done():
		return false
	case <-time.After(defaultMQTTTimeout):
		return false
	}
}
